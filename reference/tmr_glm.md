...
fit to `tmr_partition` and `tmr_absences` with the formula
...
[`build_temporal_glm`](https://cjhughes926.github.io/TemporalModelR/reference/build_temporal_glm.md),
containing `$models`, `$thresholds`, `$threshold_method`,
`$model_formula`, `$link`, `$model_vars`, `$fold_training_data`,
`$fold_test_metrics`, `$output_dir`, `$model_type`, and `$plots`.
