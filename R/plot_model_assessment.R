...
#' visualizations of sensitivity, cumulative binomial probability, G-area
#' predictions, and hypervolume size trends.
#'
...
#' @param cbp_threshold Numeric. Threshold for cumulative binomial probability
...
#' \itemize{
#'   \item volume_sensitivity_plot: ggplot object showing sensitivity trends in
#'   both e- and g- space, and proportion of study area predicted as suitable,
#'   across time.
...
#' \enumerate{
#'   \item Combined plot of proportion of study area predicted as suitable
#'   and model sensitivities over time with dual y-axes
#'   \item True positives vs false negatives in geographic space
#'   \item Cummualitve binomial probability trends on log scale with threshold
#'     reference
#' }
#'
...
plot_model_assessment <- function(data_file_path,
  ...
  plot1 <- ggplot() +
    geom_line(data = all_model_data,
              ...
              color = "#9467BD", alpha = 0.3, linewidth = 0.5, linetype = "dotted") +
    ...
    geom_line(data = summary_stats,
              ...
              color = "#9467BD", linewidth = 1, linetype = "dotted") +
    geom_line(data = all_model_data,
              aes(x = !!sym(time_column), y = G_volume, group = Fold),
              color = "#0072B2", alpha = 0.3, linewidth = 0.5) +
    geom_point(data = all_model_data,
    ...
    geom_line(data = all_model_data,
              ...
              color = "#D55E00", alpha = 0.3, linewidth = 0.5) +
    geom_point(data = all_model_data,
  ...
  plot_tp_fn <- ggplot() +
    ...
    geom_bar(data = tp_fn_summary,
             ...
             stat = "identity",
             fill = "#0072B2", alpha = 0.7, width = 0.8, color = "black", linewidth = 0.3) +
    geom_bar(data = tp_fn_summary,
             aes(x = !!sym(time_column), y = Mean_FN_G_neg),
             stat = "identity",
             fill = "#D55E00", alpha = 0.7, width = 0.8, color = "black", linewidth = 0.3) +
  ...
  if (separate_cbp) {
    ...
    plot2 <- ggplot() +
      ...
      geom_line(data = all_model_data,
                ...
                color = "gray60", alpha = 0.3, linewidth = 0.5) +
      geom_point(data = all_model_data,
    ...
    plot2 <- ggplot() +
      ...
      geom_line(data = all_model_data,
                aes(x = !!sym(time_column), y = CBP_test_E, group = Fold),
                color = "#9467BD", alpha = 0.2, linewidth = 0.5, linetype = "dashed") +
      geom_point(data = all_model_data,
                 aes(x = !!sym(time_column), y = CBP_test_E),
                 color = "#9467BD", alpha = 0.3, size = 1) +
      geom_line(data = summary_stats,
                aes(x = !!sym(time_column), y = Mean_CBP_E),
                color = "#9467BD", linewidth = 1, linetype = "dashed") +
      geom_point(data = summary_stats,
                 aes(x = !!sym(time_column), y = Mean_CBP_E),
