...
objects from each phase of the workflow. Reading the data set vignette
...
[`raster_align()`](reference/raster_align.md) standardizes rasters to a
...
pseudoreplication from your data but preserving truly unique temporally
...
comparable range and contribute evenly to subsequent modeling algorithms
sensitive to that.
...
- **[`build_temporal_glm()`](reference/build_temporal_glm.md)** fits one
  generalized linear model per fold with a binomial link function.
...
- **[`build_temporal_gam()`](reference/build_temporal_gam.md)** fits one
  generalized additive model per fold via
...
first-order (queen’s case) neighbors predicted as suitable as an
...
summarizing losses and gains in suitability over time across a these
